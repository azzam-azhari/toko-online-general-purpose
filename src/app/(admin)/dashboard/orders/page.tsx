...
export default async function OrdersPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string; payment?: string; page?: string }> }) {
  ...
  return <div className="space-y-6"><PageHeader action={<Button asChild><Link href="/dashboard/orders/new"><Plus aria-hidden="true" />Catat Pesanan</Link></Button>} description="Pantau pelanggan, pembayaran, item, dan progres pemenuhan pesanan eksternal." eyebrow="Operasional" title="Pesanan" />
    <Card><CardContent className="p-4"><form className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_180px_180px_auto]" method="get"><div className="relative"><Search aria-hidden="true" className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input className="pl-9" defaultValue={params.q} name="q" placeholder="Nomor pesanan, pelanggan, atau telepon..." /></div><Select defaultValue={status} name="status"><SelectTrigger className="h-11 w-full"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">Semua status order</SelectItem><SelectItem value="pending">Menunggu</SelectItem><SelectItem value="confirmed">Dikonfirmasi</SelectItem><SelectItem value="processing">Diproses</SelectItem><SelectItem value="shipped">Dikirim</SelectItem><SelectItem value="completed">Selesai</SelectItem><SelectItem value="cancelled">Dibatalkan</SelectItem></SelectContent></Select><Select defaultValue={payment} name="payment"><SelectTrigger className="h-11 w-full"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">Semua pembayaran</SelectItem><SelectItem value="unpaid">Belum dibayar</SelectItem><SelectItem value="pending">Menunggu bayar</SelectItem><SelectItem value="paid">Lunas</SelectItem><SelectItem value="failed">Gagal</SelectItem><SelectItem value="expired">Kedaluwarsa</SelectItem><SelectItem value="refunded">Dikembalikan</SelectItem></SelectContent></Select><Button type="submit" variant="secondary">Terapkan</Button></form></CardContent></Card>
    {orders.length === 0 ? <EmptyState description={params.q || status !== "all" || payment !== "all" ? "Coba ubah kata kunci atau filter." : "Catat pesanan setelah pelanggan memesan melalui WhatsApp atau tautan eksternal."} icon={PackageSearch} title="Pesanan tidak ditemukan" /> : <div className="overflow-hidden rounded-xl border bg-card"><div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="border-b bg-secondary/70 text-xs uppercase text-muted-foreground"><tr><th className="px-4 py-3">Pesanan</th><th className="px-4 py-3">Pelanggan</th><th className="px-4 py-3">Kanal</th><th className="px-4 py-3">Total</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Pembayaran</th><th className="px-4 py-3 text-right">Aksi</th></tr></thead><tbody className="divide-y">{orders.map((order) => <tr className="hover:bg-secondary/30" key={order.id}><td className="px-4 py-3"><Link className="font-semibold hover:text-primary hover:underline" href={`/dashboard/orders/${order.id}`}>{order.order_number}</Link><time className="mt-1 block text-xs text-muted-foreground">{new Intl.DateTimeFormat("id-ID", { dateStyle: "medium", timeStyle: "short", timeZone: "Asia/Jakarta" }).format(new Date(order.created_at))}</time></td><td className="px-4 py-3"><strong>{order.customer_name}</strong><span className="block text-xs text-muted-foreground">{order.customer_phone}</span></td><td className="px-4 py-3">{order.sales_channel === "whatsapp" ? "WhatsApp" : "Tautan eksternal"}</td><td className="px-4 py-3 font-semibold">{formatRupiah(order.grand_total)}</td><td className="px-4 py-3"><OrderStatusBadge status={order.status} /></td><td className="px-4 py-3"><PaymentStatusBadge status={order.payment_status} /></td><td className="px-4 py-3 text-right"><Button asChild size="sm" variant="outline"><Link href={`/dashboard/orders/${order.id}`}>Lihat Detail</Link></Button></td></tr>)}</tbody></table></div></div>}
