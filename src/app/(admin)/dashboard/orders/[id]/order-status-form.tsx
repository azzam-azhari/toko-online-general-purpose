...
export function OrderStatusForm({ id, status }: { id: string; status: OrderStatus }) {
  ...
  return <form className="grid gap-4" onSubmit={submit}><div><Label htmlFor="next-status">Status berikutnya</Label><Select id="next-status" onChange={(event) => setNextStatus(event.target.value as OrderStatus)} value={nextStatus}><option value="">Pilih status</option>{allowed[status].map((option) => <option key={option} value={option}>{labels[option]}</option>)}</Select></div><div><Label htmlFor="status-note">Catatan perubahan</Label><Textarea id="status-note" onChange={(event) => setNote(event.target.value)} placeholder={nextStatus === "cancelled" ? "Alasan pembatalan wajib diisi" : "Opsional, contoh: Paket diserahkan ke kurir"} value={note} /></div><Button disabled={pending || !nextStatus || (nextStatus === "cancelled" && !note.trim())} type="submit">{pending ? <Loader2 aria-hidden="true" className="animate-spin" /> : null}Perbarui Status</Button></form>;
}
