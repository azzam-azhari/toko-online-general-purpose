import { Activity, Check, Circle, Package, PackageCheck, Plus, ReceiptText, TriangleAlert, WalletCards } from "lucide-react";
import type { Metadata } from "next";
import Link from "next/link";

import { ActivityList } from "@/components/common/activity-list";
import { PageHeader } from "@/components/common/page-header";
import { SalesChart } from "@/components/common/sales-chart";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { requireAdmin } from "@/lib/auth/require-admin";
import { formatRupiah } from "@/lib/money";
import { getDashboardOverview } from "@/lib/repositories/catalog.repository";
import { getSalesOverview } from "@/lib/repositories/operations.repository";

export const metadata: Metadata = { title: "Dashboard", robots: { index: false, follow: false } };

export default async function DashboardPage() {
  const [{ profile }, overview, sales] = await Promise.all([requireAdmin(), getDashboardOverview(), getSalesOverview(14)]);
  const metrics = [
    ...
    { label: "Pesanan 14 hari", value: sales.totalOrders, detail: "Semua status", icon: ReceiptText },
    { label: "Pendapatan 14 hari", value: formatRupiah(sales.paidRevenue), detail: "Pembayaran lunas", icon: WalletCards },
  ];
  const checklist = [
  ...
  return (
    <div className="space-y-6">
      ...
      <Card><CardHeader className="flex-row items-center justify-between space-y-0"><div><CardTitle className="font-sans text-xl">Grafik Penjualan</CardTitle><CardDescription>Pendapatan harian selama 14 hari terakhir.</CardDescription></div><Button asChild size="sm" variant="ghost"><Link href="/dashboard/orders">Lihat Pesanan</Link></Button></CardHeader><CardContent><SalesChart data={sales.chart} /></CardContent></Card>
