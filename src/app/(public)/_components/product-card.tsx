...
import { buildWhatsAppUrl, formatRupiah, getDiscountPercentage } from "@/lib/storefront";
...
export function ProductCard({
  ...
  const productUrl = new URL(`/products/${product.slug}`, appUrl).toString();
  const ctaHref =
    product.cta_type === "custom_url"
      ? product.custom_url
      : product.cta_type === "whatsapp"
        ? buildWhatsAppUrl({
            ...
            settings,
            productUrl,
            fallbackNumber: process.env.NEXT_PUBLIC_DEFAULT_WHATSAPP_NUMBER,
          })
        : null;
