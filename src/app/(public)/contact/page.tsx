...
import { Camera, Clock3, Mail, MapPin, MessageCircle, Phone, Share2 } from "lucide-react";
...
export default async function ContactPage() {
  ...
  const businessHours = typeof settings.business_hours?.summary === "string" ? settings.business_hours.summary : null;
  const contacts = [
  ...
  return <InformationPage description="Gunakan saluran resmi di bawah ini untuk pertanyaan tentang produk atau informasi toko." eyebrow="Kontak" title={`Hubungi ${settings.store_name}.`}>
    ...
    <div className="mt-8 grid gap-4 md:grid-cols-3">{settings.address ? <Card><CardContent className="p-6"><MapPin aria-hidden="true" className="size-5 text-primary" /><h2 className="mt-4 font-bold">Alamat</h2><p className="mt-2 text-sm leading-7 text-muted-foreground">{settings.address}</p></CardContent></Card> : null}{businessHours ? <Card><CardContent className="p-6"><Clock3 aria-hidden="true" className="size-5 text-primary" /><h2 className="mt-4 font-bold">Jam Operasional</h2><p className="mt-2 text-sm leading-7 text-muted-foreground">{businessHours}</p></CardContent></Card> : null}<Card><CardContent className="p-6"><h2 className="font-bold">Media sosial</h2><div className="mt-4 flex flex-wrap gap-2">{settings.facebook_url ? <Button asChild variant="outline"><a href={settings.facebook_url} rel="noreferrer" target="_blank"><Share2 aria-hidden="true" /> Facebook</a></Button> : null}{settings.instagram_url ? <Button asChild variant="outline"><a href={settings.instagram_url} rel="noreferrer" target="_blank"><Camera aria-hidden="true" /> Instagram</a></Button> : null}{!settings.facebook_url && !settings.instagram_url ? <p className="text-sm text-muted-foreground">Tautan media sosial belum tersedia.</p> : null}</div></CardContent></Card></div>
  </InformationPage>;
}
