  ...
  return (
    <main>
        ...
        <section className="mx-auto max-w-7xl px-4 pt-12 sm:px-6 lg:px-8">
          <div className="grid gap-4 md:grid-cols-2">
            {banners.slice(0, 2).map((banner) => (
              <Card className="relative isolate overflow-hidden border-0 bg-primary text-primary-foreground" key={banner.id}>
      ...
      <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8" id="kategori">
        ...
        {categories.length ? (
          <div className="mt-9 grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-4">
            {categories.slice(0, 8).map((category, index) => (
              <Link className="group rounded-2xl border bg-card p-5 transition hover:-translate-y-1 hover:border-primary/25 hover:shadow-lg" href={`/products?category=${category.slug}`} key={category.id}>
        ...
        ) : <p className="mt-8 rounded-2xl border border-dashed p-8 text-center text-sm text-muted-foreground">Kategori aktif belum tersedia.</p>}
      </section>
      ...
      {testimonials.length ? (
        <section className="border-y bg-[#e9f0ea] py-20"><div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"><p className="text-sm font-bold uppercase tracking-[.16em] text-accent">Testimoni</p><h2 className="mt-2 font-serif text-4xl">Cerita dari pelanggan.</h2><div className="mt-9 grid gap-4 md:grid-cols-3">{testimonials.slice(0, 3).map((testimonial) => <Card className="border-0" key={testimonial.id}><CardContent className="p-6">{testimonial.rating ? <div aria-label={`Rating ${testimonial.rating} dari 5`} className="flex gap-1 text-[#c98216]">{Array.from({ length: testimonial.rating }, (_, i) => <Star aria-hidden="true" className="size-4 fill-current" key={i} />)}</div> : null}<blockquote className="mt-4 text-sm leading-7 text-muted-foreground">“{testimonial.quote}”</blockquote><p className="mt-5 font-bold">{testimonial.author_name}</p>{testimonial.author_title ? <p className="text-xs text-muted-foreground">{testimonial.author_title}</p> : null}</CardContent></Card>)}</div></div></section>
      ) : null}
